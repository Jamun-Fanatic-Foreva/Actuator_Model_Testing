...
def getTimeArr(duty_cycle):
    if np.shape(duty_cycle) == ():
        ...
        return getTimeArr_Vector(duty_cycle)

def getTimeArr_Vector(duty_cycle_vector):
    x, y, z = duty_cycle_vector
    x_new = getTimeArr(x)
    y_new = getTimeArr(y)
    z_new = getTimeArr(z)
    temp = np.unique(np.concatenate((x_new, y_new, z_new)))
    return temp
